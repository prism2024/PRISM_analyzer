...
module Node = struct
  ...
  let get_instr_list node = Instrs.fold ~init:[] ~f:(fun acc instr -> instr::acc) (get_instrs node)
  ...
  let get_description pe node =
    let str_kind =
      ...
      | Stmt_node node_kind ->
          "Instructions " ^ (F.asprintf "%a" pp_stmt node_kind)
...
let pp_nodeset fmt node_set = 
  IContainer.pp_collection ~fold:List.fold ~pp_item:Node.pp fmt (NodeSet.elements node_set)
