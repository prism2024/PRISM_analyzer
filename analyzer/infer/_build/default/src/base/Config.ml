...
let aprval_summary_dir = "aprval-summaries"

let aprval_result_dir = "aprval-results"

let aprval_debug_dir = "aprval-debug"
...
let () =
  let on_unknown_arg_from_command (cmd : InferCommand.t) =
    ...
    | Report ->
        `Add
    | Analyze | AnalyzeJson | Capture | Compile | Debug | Explore | Help | ReportDiff | Run | APRVAL
      ->
        `Reject
  in
...
and ( biabduction_write_dotty
  ...
  let all_generic_manuals =
    List.filter_map InferCommand.all_commands ~f:(fun (command : InferCommand.t) ->
        ...
        | Debug | Explore | Help ->
            None
        | (Analyze | AnalyzeJson | Capture | Compile | Report | ReportDiff | Run | APRVAL) as
          command ->
...
and aprval_patch_dir =
  CLOpt.mk_path ~long:"patch-dir" ~default:"patch.patch"
    ~in_help:InferCommand.[(APRVAL, manual_generic)]
    "Specify path of diff file representing patch info"


and aprval_preproc_org =
  CLOpt.mk_bool ~default:false ~long:"preproc-org"
    ~in_help:InferCommand.[(APRVAL, manual_generic)]
    "Preprocessing of original program for analysis"


and aprval_preproc_patch =
  CLOpt.mk_bool ~default:false ~long:"preproc-patch"
    ~in_help:InferCommand.[(APRVAL, manual_generic)]
    "Preprocessing of patch program for analysis"


and aprval_launch_spec_inference =
  CLOpt.mk_bool ~default:false ~long:"spec-inference"
    ~in_help:InferCommand.[(APRVAL, manual_generic)]
    "Launch specification inference"


and aprval_launch_validate =
  CLOpt.mk_bool ~default:false ~long:"spec-validate"
    ~in_help:InferCommand.[(APRVAL, manual_generic)]
    "Launch patch validation"


and aprval_test_methods =
  CLOpt.mk_string_list ~long:"test-methods"
    ~in_help:InferCommand.[(APRVAL, manual_generic)]
    "Specify the failing test-methods that trigger errors"
...
and aprval_patch_dir = !aprval_patch_dir

and aprval_preproc_org = !aprval_preproc_org

and aprval_preproc_patch = !aprval_preproc_patch

and aprval_launch_spec_inference = !aprval_launch_spec_inference

and aprval_launch_validate = !aprval_launch_validate

and aprval_test_methods = RevList.to_list !aprval_test_methods
...
let aprval_trace_dir = results_dir ^/ "trace-out"

let aprval_dyninfo_br = "BranchTaken.csv"

let aprval_dyninfo_invo = "InvokedMethodClass.csv"

let aprval_dyninfo_die = "InvokedWell.csv"

let aprval_dyninfo_oob = "ArrayAccessedWell.csv"
