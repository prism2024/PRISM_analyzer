...
module OnDisk = struct
  ...
  let load_summary_to_spec_table proc_name =
    let summ_opt =
      ...
      | None when Config.aprval_launch_validate ->
          Filename.concat Config.aprval_result_dir (specs_filename proc_name)
        ...
        | None ->
            let spec_file =  in
            if Sys.is_file_exn spec_file then 
            else None )
            *)
      | summ_opt ->
          (* TODO for incremental analysis *)
          summ_opt
    in
  ...
  let store (summary : t) =
    ...
    if Config.aprval_launch_spec_inference then
      DB.filename_from_string (Config.aprval_summary_dir ^/ specs_filename proc_name)
