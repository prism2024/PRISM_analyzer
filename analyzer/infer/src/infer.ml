...
let setup () =
  ...
  | APRVAL ->
      ( match Sys.is_directory Config.aprval_summary_dir with
      | `Yes when Config.aprval_launch_spec_inference ->
          Utils.rmtree Config.aprval_summary_dir ;
          Utils.create_dir Config.aprval_summary_dir
      | `Yes ->
          ()
      | `No ->
          Utils.create_dir Config.aprval_summary_dir
      | `Unknown ->
          Utils.create_dir Config.aprval_summary_dir ) ;
      ( match Sys.is_directory Config.aprval_result_dir with
      | `Yes when Config.aprval_launch_spec_inference ->
          Utils.rmtree Config.aprval_result_dir ;
          Utils.create_dir Config.aprval_result_dir
      | `Yes ->
          ()
      | `No ->
          Utils.create_dir Config.aprval_result_dir
      | `Unknown ->
          Utils.create_dir Config.aprval_result_dir ) ;
      ( match Sys.is_directory Config.aprval_debug_dir with
      | `Yes when Config.aprval_launch_spec_inference ->
          Utils.rmtree Config.aprval_debug_dir ;
          Utils.create_dir Config.aprval_debug_dir
      | `Yes ->
          ()
      | `No ->
          Utils.create_dir Config.aprval_debug_dir
...
let preproc () =
  L.progress "Extract call graph@." ;
  let program = Program.from_marshal ~force:true () in
  L.progress "Get dyninfo@." ;
  let dyninfo = DynInfo.parse_dyninfo program in
  Vocab.print_to_file ~dirname:(Some Config.results_dir)
    ~msg:(F.asprintf "%a" DynInfo.pp dyninfo)
    ~filename:"dyninfo.results" ;
  L.progress "Patch parsing@." ;
  let patch = Patch.from_marshal ~force:true () in
  L.progress "%a@." Patch.pp patch ;
  (* Precisely check the patch point by comparing deletied and inserted lines *)
  if Config.aprval_preproc_patch then (
    let org_patch_path = Filename.concat "infer-out-origin" "patch.data" in
    let patch_org = Utils.with_file_in org_patch_path ~f:Marshal.from_channel in
    let patch_org', patch' = Patch.preproc patch_org patch in
    L.progress "Patch info org' : %a@, Patch info after patch': %a@." Patch.pp patch_org' Patch.pp
      patch' ;
    Patch.to_marshal org_patch_path patch_org' ;
    Patch.to_marshal Patch.marshalled_path patch' ;
    Vocab.print_to_file ~dirname:(Some "infer-out-origin")
      ~msg:(F.asprintf "%a" Patch.pp patch_org')
      ~filename:"patch.results" ;
    Vocab.print_to_file ~dirname:(Some Config.results_dir) ~msg:(F.asprintf "%a" Patch.pp patch')
      ~filename:"patch.results" ;
    Vocab.print_to_file ~dirname:(Some Config.results_dir) ~msg:(Patch.export_target_info patch')
      ~filename:"patch_mthds.results" ;
    () ) ;
  ()
  ...
  | Explore ->
      if (* explore bug traces *)
        ...
        TraceBugs.explore ~selector_limit:None ~report_json:(ResultsDir.get_path ReportJson)
          ...
          ~show_source_context:Config.source_preview ~max_nested_level:Config.max_nesting
  | APRVAL ->
      if Config.aprval_preproc_org || Config.aprval_preproc_patch then preproc ()
      else
        (* Get summary of original program from pre-analyzed data *)
        let get_summary_from_file proc_name =
          let original_summary_path =
            Procname.to_filename proc_name ^ ".specs"
            |> Filename.concat Config.aprval_summary_dir
          ...
          | _ ->
              ...
              None
        in
        let get_summary_from_memory proc_name =
          match Summary.OnDisk.get proc_name with
          | Some Summary.{payloads= {spec_checker= Some spec_checker_summary}} ->
              Some spec_checker_summary
          | _ ->
              ...
              None
        in
        (* Load the target program then sclicing meaningless methods and cfg node using slicing *)
        let program = Program.from_marshal () in
        let patch = Patch.from_marshal () in
        let target_procs = Patch.get_target_procs patch in
        let related_procs =
          Program.cg_reachables_of program target_procs
          |> Procname.Set.filter (fun proc -> not (Vocab.is_non_target_proc proc))
        in
        let skipped_nodes = Preproc.SkippedNodes.create program target_procs in
        L.progress "Program slicing@." ;
        Program.slice_procs_except program related_procs ;
        Program.to_marshal Program.marshalled_path program ;
        Program.print_callgraph program "callgraph_sliced.dot" ;
        L.progress "Patch mthd info : %s@." (Patch.export_target_info patch) ;
        L.progress "Patch Info : %a@." Patch.pp patch ;
        L.progress "Target proc info : %a @." Procname.Set.pp target_procs ;
        L.progress "Nodes removed : %a @." Preproc.SkippedNodes.pp skipped_nodes ;
        L.progress "# of Callees : %d @." (Procname.Set.cardinal related_procs) ;
        (* spec inference : analyze original program and save the summries as files *)
        if Config.aprval_launch_spec_inference then (
          InferAnalyze.main ~changed_files:None ;
          (* Saved the results which will be unchanged after patch only *)
          let unchanged =
            Procname.Set.diff related_procs
              (Program.cg_reachables_of ~forward:false program target_procs)
          in
          Procname.Set.iter
            (fun procname ->
              ...
              | Some summary ->
                  let serializer = Serialization.create_serializer Serialization.Key.summary in
                  DB.filename_from_string
                    (Config.aprval_result_dir ^/ Procname.to_filename procname ^ ".specs")
                  |> Serialization.write_to_file serializer ~data:summary
              | _ ->
                  L.(die ExternalError)
                    "%a has not been analyzed during inference" Procname.pp procname)
            unchanged ;
          L.progress "[SUCCESS] analyze org@." ;
          (* Store the original result *)
          let spec_buggy = StateFormula.SpecMap.create ~get_summary:get_summary_from_memory in
          StateFormula.SpecMap.to_marshal spec_buggy ;
          Vocab.report_msg_debugger () ;
          L.exit 0 )
        else if Config.aprval_launch_validate then (
          L.progress "Analyze patch program@." ;
          InferAnalyze.main ~changed_files:None ;
          PatchValidator.run ~get_buggy_summary:get_summary_from_file
            ~get_patch_summary:get_summary_from_memory ;
          L.progress "[SUCCESS] validate patch@." ;
          L.exit 0 )
        else L.progress "[FAIL]: to analyze error proc@." ) ;
