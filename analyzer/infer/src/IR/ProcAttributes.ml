...
let pp_var_data fmt {name; typ; modify_in_block; is_constexpr; is_declared_unused} =
  F.fprintf fmt
    "@[<h>{ name=@ %a;@ typ=@ %a;@ modify_in_block=@ %b;@ is_constexpr=@ %b;@ is_declared_unused=@ \
     %b@ }@]"
    Mangled.pp name (Typ.pp_full Pp.text) typ modify_in_block is_constexpr is_declared_unused
