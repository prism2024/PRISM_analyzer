...
class Ret:
    def __init__(self, stdout, stderr, return_code, time):
        try:
            ...
            self.stdout = stdout.decode(encoding="ISO-8859-1")
            self.stderr = stderr.decode(encoding="ISO-8859-1")
...
def execute(cmd, dir=None, env=None, timeout=1200, verbosity=0):
    ...
    try:
        ...
        print(f"{TIMEOUT} occurs during executing {cmd} at {dir}")
        stdout, stderr = b"", b""
        returncode = 124
    except OSError:
...
def dict_leaves_to_string(target):
    if type(target) is not dict:
        return f",{target}"

    result = ""
    for key, value in target.items():
        result += dict_leaves_to_string(value)
